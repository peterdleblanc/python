...
def get_usgs_eq_data():
    ...
    response = urllib2.urlopen('http://earthquake.usgs.gov/fdsnws/event/1/query.geojson?starttime=2015-01-10%2000:00:00&minmagnitude=3&endtime=2015-05-30%2023:59:59&orderby=time')

    logger.info('###############Query Range: 2015-05-01 to  2015-05-30######################################')
    ...
    for event in list_of_events:
        ...
        logger.info('adding event' + data['id'] + 'to solr')
        s.add(id=seq_id,event_id=data['id'],event_time=data['properties']['time'],title=title, place=place)
        s.commit()
